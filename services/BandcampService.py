...
class BandcampService:
    ...
    def get_weekly_shows(self, shows: str) -> dict:
        shows_list = {}
        valid_shows = self._get_valid_shows(shows, self._get_available_shows())

        if len(valid_shows) != 0:
            for sh in valid_shows:
                show_obj = self._get_weekly_show(sh)
                shows_list[sh] = show_obj
        else:
            raise HTTPException(status_code=404, detail="Valid Bandcamp Weekly Show ID not found")
        return shows_list

    def _get_weekly_show(self, show_id: str):
        if show_id not in self._get_available_shows():
    ...
    @staticmethod
    def _get_valid_shows(shows: str, available_shows: list) -> list:
        valid_shows = []
    ...
    def get_genre_essentials_list(self, genres: str) -> dict:
        genre_essentials = {}
        valid_genres = self._get_valid_genres(genres)
        if len(valid_genres) != 0:
            for genre in valid_genres:
                genre_ess_obj = self._get_genre_essentials(genre, Constants.ESSENTIALS_TAB)
    ...
    @staticmethod
    def _get_genre_essentials(genre: str, tab: str) -> list:
    ...
    @staticmethod
    def _get_valid_genres(genres: str) -> list:
        valid_genres = []
    ...
    def get_genre_highlights_list(self, genres: str) -> dict:
        ...
        if len(valid_genres) != 0:
            for genre in valid_genres:
                genre_ess_obj = self._get_genre_essentials(genre, Constants.HIGHLIGHTS_TAB)
    ...
    def get_album_of_day(self, day: str):
        ...
        self._check_day_format(day)
        return self._search_album_of_day(day)

    @staticmethod
    def _check_day_format(day: str):
        try:
            datetime.strptime(day, '%d-%m-%Y')
        except ValueError:
            raise HTTPException(status_code=404, detail="Incorrect data format, should be DD-MM-YYYY")

    def _search_album_of_day(self, day: str):
        page = self._estimate_page_search(day, "1")
        albums_list = ScrapeService.scrape_aotd_page(Constants.album_of_day_endpoint(page))
        album_of_day = self._filter_album_by_day(albums_list, day)
        return DataLoadService.load_album_of_day(album_of_day)

    # TODO: refactor
    def _estimate_page_search(self, day: str, page) -> str:
        last_day_of_page = self._get_last_day_of_page(
    ...
    def _get_last_day_of_page(self, aots):
        return self._date_formatter(aots[-1][0])

    def _filter_album_by_day(self, albums, day):
        for aotd in albums:
            if day == self._date_formatter(aotd[0]):
                return aotd
        raise HTTPException(status_code=404, detail=f"Album of day: {day} not found")

    @staticmethod
    def _date_formatter(day: str) -> str:
